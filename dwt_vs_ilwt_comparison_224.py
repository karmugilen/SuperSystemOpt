    ...
    def forward(self, x):
        ...
        scale = torch.cat(
            [
                ...
                torch.full_like(cover_ycc[:, 1:2], kC),
                torch.full_like(cover_ycc[:, 2:3], kC),
            ],
            dim=1,
        )
    ...
    def inverse(self, z):
        ...
        scale = torch.cat(
            [
                ...
                torch.full_like(z_ycc[:, 1:2], kC),
                torch.full_like(z_ycc[:, 2:3], kC),
            ],
            dim=1,
        )
...
def steganography_loss(
    ...
    sh_dx, sh_dy = gradients(stego_img)
...
def ilwt_multiscale_secret_loss(ilwt_module, secret_img, recovered_secret):
    """
    Multi-scale loss prioritizing LL (structure), then LH/HL (edges), least HH.
    Also adds a second-level LL-on-LL constraint to strongly preserve structure.
    """
    # Level-1 bands
    ...
    # Knowledge-driven weights (LL highest, HH lowest) - INCREASED for stronger structure
    wLL, wLH, wHL, wHH = 0.35, 0.14, 0.14, 0.06  # Doubled from 0.18, 0.07, 0.07, 0.03
    loss_lvl1 = (
    ...
    # Level-2: enforce structure on LL via LL-of-LL - INCREASED
    ...
    wLL2 = 0.20  # Doubled from 0.10 for deeper structure preservation
...
def train_model(model, train_dataset, val_dataset, num_epochs=100, save_metrics=True, validate_every=5):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (input_tensor, host_tensor, secret_tensor) in enumerate(train_dataloader):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                # Curriculum for perturbation: ramp from off -> on (START LATER for better learning)
                ramp = min(1.0, max(0.0, (epoch / max(1, int(0.85 * num_epochs))) - 0.0))  # Was 0.7
                q_prob = 0.0 + 0.15 * ramp  # Reduced from 0.2 (less aggressive)
                noise_prob = 0.0 + 0.15 * ramp  # Reduced from 0.2
                noise_sigma = 0.002 + (0.003 - 0.002) * ramp  # Reduced from 0.003-0.005
                stego_host_pert = perturb_stego_like_for_inverse(
                    stego_host,
                    q_prob=q_prob,
                ...
                stego_like = torch.cat(
                    [stego_host_pert, torch.zeros_like(stego_host_pert)], dim=1
                )
                reconstructed_input = model.inverse(stego_like)
                recovered_secret = reconstructed_input[:, 3:, :, :]

                # Linearly ramp alpha_hid across epochs
                ...
                loss = (loss + ms_loss) / accumulation_steps  # Scale loss for gradient accumulation
        ...
        if epoch % validate_every == 0:  # Validate every 'validate_every' epochs
            ...
            with torch.no_grad():
                for val_batch_idx, (input_tensor, host_tensor, secret_tensor) in enumerate(val_dataloader):
                    ...
                    total_val_loss = val_loss + val_ms_loss
                    ...
                    val_rec_ssim_val = 1.0 - val_rec_ssim.item()  # Convert loss back to SSIM
        ...
        metrics_history['alpha_hid'].append(alpha_hid)
...
def generate_research_plots(metrics_history):
    ...
    plt.subplot(2, 3, 6)
    plt.plot(epochs, metrics_history['alpha_hid'], 'k-', linewidth=2)
